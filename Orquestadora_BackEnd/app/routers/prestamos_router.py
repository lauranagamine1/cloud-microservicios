...
# 🔥 Cambiado al ALB público + PUERTOS
PRESTAMOS_API_URL = "http://prestamos_api:5101/loans"
...
def rent_book(data: RentRequest):
    resp = requests.get(f"{PRESTAMOS_API_URL}/user/{data.user_id}/active")
    ...
    libro_resp = requests.get(f"{LIBROS_API_URL}/{data.book_id}")
    ...
    r = requests.post(PRESTAMOS_API_URL, json=nuevo_prestamo)
    if r.status_code != 201:
        raise HTTPException(status_code=500, detail="No se pudo registrar el prestamo")

    resp2 = requests.put(f"{LIBROS_API_URL}/rent/{data.book_id}")
...
def devolver_libro(data: DevolucionRequest):
    resp_get = requests.get(f"{PRESTAMOS_API_URL}/{data.loan_id}")
    ...
    resp = requests.put(f"{PRESTAMOS_API_URL}/{data.loan_id}", json={"status": "returned"})
    if resp.status_code != 200:
        raise HTTPException(status_code=500, detail="Error al actualizar el prestamo")

    resp2 = requests.put(f"{LIBROS_API_URL}/return/{data.book_id}")
    if resp2.status_code != 200:
...
def prestamos_activos(user_id: str):
    resp = requests.get(f"{PRESTAMOS_API_URL}/user/{user_id}/active")
    if resp.status_code != 200:
