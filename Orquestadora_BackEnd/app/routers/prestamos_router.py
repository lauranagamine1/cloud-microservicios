...
def rent_book(data: RentRequest):
    resp = requests.get(f"http://localhost:5101/loans/user/{data.user_id}/active")
    ...
    libro_resp = requests.get(f"http://localhost:8001/books/{data.book_id}")
    ...
    r = requests.post("http://localhost:5101/loans", json=nuevo_prestamo)
    if r.status_code != 201:
        raise HTTPException(status_code=500, detail="No se pudo registrar el prestamo")

    resp2 = requests.put(f"http://localhost:8001/books/rent/{data.book_id}")
...
def devolver_libro(data: DevolucionRequest):
    resp_get = requests.get(f"http://localhost:5101/loans/{data.loan_id}")
    ...
    resp = requests.put(f"http://localhost:5101/loans/{data.loan_id}", json={"status": "returned"})
    if resp.status_code != 200:
        raise HTTPException(status_code=500, detail="Error al actualizar el prestamo")

    resp2 = requests.put(f"http://localhost:8001/books/return/{data.book_id}")
    if resp2.status_code != 200:
...
def prestamos_activos(user_id: str):
    resp = requests.get(f"http://localhost:5101/loans/user/{user_id}/active")
    if resp.status_code != 200:
